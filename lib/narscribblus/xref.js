...
ArgRef.prototype = {
  // this gets us traverseChild, traverseArg, and a "kind" getter
  ...
  _resolve: function() {
    ...
    var traversedThing = this.pkg.traverse(this.s);
    if (!traversedThing)
      return false;

    // pierce the type; we don't want to discuss the name of the other argument!
    this.resolved = traversedThing.type;
    if (this.resolved)
      return true;
    return false;
  },
...
/**
 * Cross-reference the doc stream of something documented elsewhere.
 */
function DocSplice(svals, tvals, ctx) {
  this.resolved = null;
  this.s = coerceString(svals[0]);
  this.pkg = ctx.options.pkg;
  if (!this.pkg)
...
DocSplice.prototype = {
  ...
  genus: "docsplice",
  isType: false,
  // the splice will have its own paragraphs built-in.
  ...
  _resolve: function() {
    ...
    this.resolved = this.pkg.traverse(this.s);
    if (this.resolved)
      return true;
    return false;
  },
  get docStream() {
    if (this._resolve())
      return this.resolved.docStream;
    return [];
  },
};
exports.DocSplice = DocSplice;
