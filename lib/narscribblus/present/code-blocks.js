...
require.def("narscribblus/present/code-blocks",
  [
    "wmsy/wmsy",
...
wy.defineWidget({
  ...
  // XXX this should really use emit to allow for crumbs and avoid nesting
  popups: {
    clickedLink: {
  ...
  // just be a div; we will fill in our children ourselves.
  structure: {
  },
  impl: {
  ...
  events: {
    root: {
      click: function(targetBinding, event) {
        ...
        // XXX do something to fancy up the thing we clicked on
        this.popup_clickedLink(linked, this, function done() {
          // XXX clear the fanciness
        });
