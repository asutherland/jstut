 ...
 *
 * The forms we do not recognize and it may be concerning:
 * - identifiers.  In theory we could detect these and then create a function
 *    that is either handed a reference to the module 'global' so that we can
 *    fake lexically grab (ex: globy[attrName]) or even inject JS code into
 *    an eval'ed block of code that does what we want.  Need to figure out
 *    in what context this makes sense and whether something else makes more
 *    sense.
 *
 * The forms we do not recognize and are silly (but could be useful):
 * - symbols ('symbol, '|quoted symbol|)
 * - keywords (#:keyword); used in argument lists...
 * - cons pairs: '(1 . 2)
 * - lists: '(1 2 3)
 * - vectors: '#(1 2 3)
 *
...
function atParser(s, ctx, idx, re_at_or_term) {
  ...
  if (idx < slen && s[idx] == "|") {
    ...
    if (RE_ALT_SYNTAX_PUNC.test(s[idx + 1])) {
      ...
      idx = idxSquiggly + 1;
      cmdList[2] = textStreamAtBreaker(s.substring(idx), ctx, altSynRe);
    ...
    if (cmdList[0] && (cmdList[0] in ctx.readerMap)) {
      var readerFunc = ctx.readerMap[cmdList[0]];
      var rfr = readerFunc(s.substring(idx + 1), ctx, cmdList[1]);
      if (rfr[0] !== undefined)
        results.push(rfr[0]);
      idx += rfr[1];
    }
    else {
      cmdList[2] = textStreamAtBreaker(s.substring(idx + 1), ctx, RE_AT_TEXT);
