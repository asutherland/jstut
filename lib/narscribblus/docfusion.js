...
DocFusion.prototype = {
  /**
   * Overlapped bootstrap not supported; we really need promises...
   */
  requireModule: function(name, callback) {
    var pkgName = name.split("/")[0];
    if (pkgName in this.packages) {
      return this.packages[pkgName].requireModule(name, callback);
    }

    var pkgFusion = this.packages[pkgName] = new PackageFusion(name);
    return pkgFusion.bootstrap(function() {
      pkgFusion.requireModule(name, callback);
    });
  },
};
