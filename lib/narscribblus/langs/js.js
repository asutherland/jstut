...
exports.expand = function expand(parsed, ctx) {
  ...
  minfo.exportNS = munge.mungeNamespace("exports", scopes.exports);
  minfo.globalNS = munge.mungeNamespace("global", scopes.global);
...
DocNode.prototype = {
  ...
  toHTMLString: function(options) {
    var s = "";
    if (this.sym) {
      s = "<h2>" + this.sym.name + "</h2>\n";
    }
    return s + html.htmlStreamify(this.textStream, options);
  }
};
...
function chewGroupLines(contentTokens, ctx) {
  var blockStr = contentTokens.map(function (t) { return t.value; }).join("\n");
  return parseDocBlockAsScribble(blockStr, ctx);
}
 ...
 * Render a namespace
 */
function htmlifyNamespace(nsName, ns, options) {
  var bits = [];
  bits.push("<h2>" + htmlEscapeText(nsName) + "</h2>\n");
  bits.push("<dl>\n");
  for (var key in ns.childrenByName) {
    bits.push("  <dt>" + htmlEscapeText(key) + "</dt>\n");
    var sym = ns.childrenByName[key];
    if ("docNode" in sym)
      bits.push("  <dd>" + sym.docNode.toHTMLString(options) + "</dd>\n");
  }
  bits.push("</dl>\n");
  return bits.join("");
}
...
function processShowModuleInfo(parsed, ctx) {
  ...
  var bits = [];
  bits.push(htmlifyNamespace("exports", minfo.exportNS, options));
  bits.push(htmlifyNamespace("module global scope", minfo.globalNS, options));
