...
var docfusion = require("narscribblus/docfusion");
var docFusion = docfusion.docFusion, ModuleInfo = docfusion.ModuleInfo;
var munge = require("narscribblus/interp-munge");

var jsdoc = require("narscribblus/langbits/jsdoc");

var uijunk = require("narscribblus/ui-junk");
...
DocNode.prototype = {
  gimmeArgTypes: function() {
    var argsNode = jsdoc.findChildByType(this.rawStream, jsdoc.Args);
    if (argsNode)
      return argsNode.kids;
    return null;
  },
  ...
  for (var i = 0; i < tokens.length; i++) {
    ...
    if (pendingDocNode) {
      switch (token.type) {
        case FUNCTION:
        case IDENTIFIER:
          console.log("assigning to", token);
...
function processInlineDocs(parsed, ctx) {
  ...
  return {
    body: html.htmlDocify(
            jsCommentSlicer(parsed.tokenizer.tokenLog, ctx),
            ctx,
            [ctx.options.popupManager.getCss()]),
    liveject: function runLivejecters(doc, win) {
      console.log("livejecting!");
