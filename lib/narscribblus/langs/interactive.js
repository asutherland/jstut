...
exports.expand = function expand(nodes, ctx) {
  ...
  var expanded = decodeFlow(man.textStreamChewer(nodes, ctx));
...
EditableCode.prototype = {
  alterCode: function(newCode) {
    ...
    for (var i = 0; i < this.updateListeners.length; i++) {
      this.updateListeners[i].codeUpdated(dis);
    }
  },

  ensureInterpreted: function() {
    if (this._interpreted)
      return;

    var preAsts = [];
    for (var i = 0; i < this.depBlockNames.length; i++) {
      var depBlockName = this.depBlockNames[i];
      preAsts.push(this.ctx.options.codeBlocks[depBlockName].code.script);
    }

    this.ctx.joinPhase(when(
      this.ctx.interpbox.processAnonSnippet(preAsts,
                                            this.code.script,
                                            this.name),
      function() {
        this._interpreted = true;
      }));
  },
