  ...
  getNamedCodeBlocks: function(names, interpreted) {
    ...
    names.forEach(function (name, index) {
      ...
      if (!(name in self[deferredAttrName])) {
        self[deferredAttrName][name] =
          pwomise.defer("codeBlock", name);
      }
      when(self[deferredAttrName][name].promise, function(codeBlock) {
    ...
    console.log("getNamed:", names, "pending", pendingCount);
    if (pendingCount)
    ...
    function(editableCodes) {
      ...
      for (var i = 0; i < tokens.length; i++) {
        ...
        if (typeof(token) !== "string") {
          ...
          if (depthStack.length &&
            ...
            if (active && effDepth == depthThresh && lastWhitespace) {
              console.log("outing", "'" + lastWhitespace + "'", out.concat());
            ...
            console.log("depth pop on", {n: depthStack[depthStack.length-1]},
                        token, curDepth);
          }
          if (token.nodeLink) {
            ...
            if (linked.type in DEPTH_INCREASING_TYPES) {
              ...
              curDepth = effDepth++;
              if (debug) out.push("+(" + effDepth + ")");
              console.log("depth push on", token, effDepth);
        ...
        if (active && typeof(token) !== "string") {
          ...
          if (token.value in expandMap) {
            ...
            if (expandMap[token.value] === true) {
              ...
              if (("interpObj" in token.nodeLink) &&
                ...
                if (expansionPayloadParseNode) {
                  expansionStack.push([expansionPayloadParseNode,
                                       expansionDepthDelta]);
                  console.log("pushed stack", expansionStack.concat());
                }
              ...
              else if (token.nodeLink.hasOwnProperty("interpObj")) {
                console.log("thing", token.nodeLink.interpObj[0]);
              }
              else {
                console.log("no interpObj on", {nl: token.nodeLink});
              }
              console.log("+expand map activating, cur depth", effDepth, token);
            }
          ...
          if (expansionPropParseNode && token.nodeLink &&
            ...
            if (("interpObj" in nodeLink) &&
              ...
              console.log(token, "nodeLink", {nl: nodeLink, ep: expansionPropParseNode});
              throw new Error("did not match up attrval with attr!");
            }
          }
          else if (expansionPropParseNode) {
          ...
          if (expansionPayloadParseNode &&
            ...
            console.log("-expand map going away", {token: token,
                                                   pn: expansionPayloadParseNode});
        ...
        if (!active) {
          ...
          if (("interpObj" in token.nodeLink) &&
              (token.nodeLink.interpObj[0] == "attr")) {
            console.log("!!! setting expansionPropParseNode");
            expansionPropParseNode = token.nodeLink;
            expansionDepthDelta = 0;
          }
          else
