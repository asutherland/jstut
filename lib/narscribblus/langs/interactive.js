...
function ExampleSlice(ctx, exampleName, rootTokenValue, depthThresh,
  ...
  var self = this;
  when(
    ctx.options.codeClearingHouse.getNamedCodeBlocks([exampleName]),
    function(editableCodes) {
      ...
      var expansionParseNode = null, expansionDepthDelta = 0;
        ...
        if (typeof(token) !== "string") {
          ...
          if (token.nodeLink) {
            ...
            if (linked.type in DEPTH_INCREASING_TYPES) {
              console.log("depth push on", token);
          ...
          if (token.value in expandMap) {
            console.log("+expand map activating, cur depth", effDepth);
            // take effect on this dude...
            if (expandMap[token.value] === true) {
              expansionParseNode = token.nodeLink;
          ...
          if (expansionParseNode &&
              token.end === expansionParseNode.end) {
            console.log("-expand map going away");
            expansionParseNode = null;
