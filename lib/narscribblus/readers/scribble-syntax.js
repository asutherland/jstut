...
function atParser(s, ctx, idx, re_at_or_term) {
  ...
  if (idx < slen && s[idx] == "|") {
    ...
    if (RE_ALT_SYNTAX_PUNC.test(s[idx + 1])) {
      ...
      if (tokens)
        tokens.push(new Token(s.substring(idx, idxSquiggly + 1), AT_LSQUIGGLE));
      idx = idxSquiggly + 1;
      cmdTextStream = textStreamAtBreaker(s.substring(idx), ctx, altSynRe);
      // token-wise, the textStreamAtBreaker is responsible for the closing bit
      //  because the alternate syntax mode has state not exposed to us (where
      //  the match starts)
