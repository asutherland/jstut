...
function munge(interpNode, modInfo, attrName, ownerStack) {
  ...
  if (!sym) {
    ...
    switch (interpNode.type) {
      case "function":
        ...
        if (hasProtoStuff)
          sym = new $typerep.ClassType(interpNode.node.name, life);
  ...
  //  check would only try and do stuff if docNodes were not already assinged.
...
var mungersByKind = {
  "object": function(sym, interpNode, modInfo, ownerStack) {
    var odata = interpNode.data, key, kidSym;
    var meStack = ownerStack.concat([this]);
    for (key in odata) {
      kidSym = munge(odata[key], modInfo, key, meStack);
      if (kidSym)
        groupyLogic(sym, key, kidSym);
    }
  ...
  "class": function(sym, interpNode, modInfo, ownerStack) {
    ...
    // - static
    for (key in sdata) {
      kidSym = munge(sdata[key], modInfo, key, meStack);
      if (kidSym)
        groupyLogic(this.staticBits, key, kidSym);
    }

    // - instance
    for (key in pdata) {
      kidSym = munge(pdata[key], modInfo, key, meStack);
      if (kidSym)
        groupyLogic(this, key, kidSym);
    }
