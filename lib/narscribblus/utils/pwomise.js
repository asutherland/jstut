...
define("narscribblus/utils/pwomise",
  [
    "exports",
...
exports.when = function (value, resolved, rejected, what, whatSpecifically) {
    ...
    forward(promisedValue, "when", function (value) {
        ...
        //  it amounts to calling the resolved handler because "ref" constructs
        //  a straight-up Promise which will invoke "resolved" after invoking
        //  the "when" handler which is just the identity function for a "ref"ed
        //  value.
        // (If the value was itself a deferred's promise, then our resolved
        //  function will instead be enqueued (if not yet resolved) or forwarded
        //  again (if already resolved).)

        // Case analysis of "value":
        // 1) It is a promise that belongs to a deferred.
        //
        //    In this case, ref(value) will just return the promise value
        //     unchanged.  The call to emit will be that of deferred, so the
        //     when/resolved/rejected will be queued if the deferred has not
        //     yet been resolved, or forward()ed if it has been resolved.
        //     The result is that the logic associated with this "when" is
        //     pushed into the future, blocking on the deferred promise returned
        //     by our promisedValue.
        //
        //    Either way, the call to emit will return undefined and that is
        //     what we will pass to
        //
        // 2) It is a value.
...
//        //retPromise.promiseDeps = deferred.promise.promiseDeps;
