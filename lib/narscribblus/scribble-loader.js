...
exports.parseDocument = function(aDocString, aFileDataPath, aOptions) {
  ...
  var expanded = when(lang.parse(body, ctx), function expander(parsed) {
    console.info("*** expand phase", parsed);
    var expanded = lang.expand(parsed, ctx);
    console.info("  * expand result:", expanded);
    return expanded;
  });
  var processed = when(expanded, function processor(expanded) {
    console.info("*** process phase", expanded);
    return lang.process(expanded, ctx);
  });
  var resPromise = pwomise.wrap(processed, "parseDocument", aFileDataPath);
  console.log("parseDocument result promise", resPromise);
  return resPromise;
};
