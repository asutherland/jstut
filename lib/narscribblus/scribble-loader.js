...
exports.parseDocument = function(aDocString, aFileDataPath, aOptions) {
  ...
  // parsing is required to be synchronous
  var parsed = lang.parse(body, ctx);
  // expansion can be asynchronous
  lang.expand(parsed, ctx);
  ctx.waitForAllPromises().then(function(expanded) {
    lang.process(expanded, ctx);
  });
};
