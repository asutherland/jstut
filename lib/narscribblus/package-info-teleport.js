...
function loadSource(aSourceRef) {
  ...
  req.addEventListener("load", function() {
    if (req.status == 200)
      deferred.resolve(aSourceRef, req.responseText);
    else
      deferred.reject(aSourceRef, req.status);
  }, false);
...
function loadData(aDataRef) {
  ...
  req.addEventListener("load", function() {
    if (req.status == 200)
      deferred.resolve(aDataRef, req.responseText);
    else
      deferred.reject(aDataRef, req.status);
  }, false);
  req.setRequestHeader("Cache-Control", "no-cache");
  req.send(null);
  return deferred.promise;
