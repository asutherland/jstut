...
function EditableCode(svals, ctx) {
  ...
  if (!("codeBlocks" in ctx.blackboard))
    ctx.blackboard.codeBlocks = {};
  ctx.blackboard.codeBlocks[this.name] = this;
}
...
function CodeExecution(blockNames, ctx) {
  ...
  for (var i = 0; i < blockNames.length; i++) {
    var edcode = ctx.blackboard.codeBlocks[blockNames[i]];
    edcodes.push(edcode);
...
function ExampleBlock(svals, ctx) {
  ...
  this.codeExec = new CodeExecution(blockNames, ctx);
}
ExampleBlock.prototype = {
  toHTMLString: function() {
    return this.editableCode.toHTMLString() +
           this.displayarea.toHTMLString();
  },
};
...
exports.narscribblusExecFuncs = {
  ...
  collapsed: function(name, svals, tvals, ctx, chewer) {
    return new man.Fragment(chewer(tvals));
  },
