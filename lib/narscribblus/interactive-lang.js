...
var render_js = require("narscribblus/renderer-js");
...
exports.expand = function expand(nodes, ctx) {
  ...
  return man.decodeFlow(man.textStreamChewer(nodes, ctx));
};
...
function EditableCode(svals, ctx) {
  ...
  var interp = new astinterp.Interpreter(this.code.script, "blah", [],
                                         {commonJS: false});
  interp.interpret();
  dump("*** INTERP " + this.name + "\n");
...
EditableCode.prototype = {
  ...
  liveject: function(doc, ctx) {
    ...
    this.bespinEnv = null;
    // fill in the text
    console.log("looking for", "edcode-doedit-" + this.name);
    ...
    editBtn.addEventListener("click", function(event) {
      ...
      edDiv.textContent = dis.code.text;
      // make it a bespin editor
      console.log("doing bespin hookup on", dis.name);
      doc.defaultView.wrappedJSObject.bespin.useBespin(
...
ExampleBlock.prototype = {
  htmlDontWrapInPara: true,
  toHTMLString: function(options) {
    if (!("interactive" in options.namedCssBlocks)) {
 ...
 * @args[
 *   @listarg["svals"
 *     @arg["label"]{The label for the collapsed block.}
 *   ]
 *
 * ]
 */
function Collapsed(svals, ctx) {
