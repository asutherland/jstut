...
define("narscribblus/narcissus/jsparse",
  [
    "exports",
    "narscribblus/narcissus/jsdefs",
    "narscribblus/narcissus/jslex",
  ],
  function (
    exports,
    m_jsdefs,
...
var Tokenizer = m_jslex.Tokenizer;
...
function Node(t, type) {
  this.__proto__ = {
    __proto__: new Array(),
    constructor: Np.constructor,
    toSource: Np.toSource,
    push: Np.push,
  };
    ...
    if (token) {
        ...
        this.end = token.end;
        // establish a back-link from the cloned token to this Node.
        // (the clone will generally have the same information as us except its
        //  token types will all be lexer token types rather than semantic
        //  types and lives in a stream of tokens for which the parser may
        //  have discarded some without building Nodes that correspond.)
        if (token.clone) // &&
            //((token.type === IDENTIFIER) || (token.type === FUNCTION) ||
            // (type === OBJECT_INIT)))
          token.clone.nodeLink = this;
        // yuck; so, in the cases where we don't care enough to immediately
        //  establish the node backlink, we still need to hold onto a reference
        //  to the lexer token so we can re-establish the link later.
        this.synToken = token.clone;
    } else {
        this.type = type;
...
function Expression(t, x, stop) {
    ...
    // Helper to avoid undefined complaints about precedence... it is
    //  conceivable we should instead be fixing up the precedence table, but
    //  this helper does not change the already existing behaviour...
    function topPrecedence() {
      var top = operators.top();
      if (!(top.type in opPrecedence))
        return undefined;
      return opPrecedence[top.type];
    }
    ...
    while ((tt = t.get()) != END) {
        ...
        switch (tt) {
          ...
          case COLON:
            ...
            while (topPrecedence() > opPrecedence[tt] ||
                    ...
                    if ((t.token.value == "get" || t.token.value == "set") &&
                        ...
                        switch (tt) {
                          ...
                          case RIGHT_CURLY:
                            ...
                            // We have now eaten the curly and are jumping out
                            //  of the if, so we need to manual perform the end
                            //  fix-up rather than rely on match/mustMatch.
                            n.end = t.token.end;
                            break object_init;
                          default:
          ...
          case LEFT_PAREN:
            if (t.scanOperand) {
                ...
                while (topPrecedence() > opPrecedence[NEW])
                    reduce();
          ...
          case RIGHT_PAREN:
            ...
            if (tt != GROUP) { // => CALL or NEW_WITH_ARGS
                n = operands.top();
                // if it's single argument, n[1] will be ')', I think...
                // (so normalize a LIST into existence since there was no comma
                //  to do it for us.)
                if (n[1].type != COMMA)
                    n[1] = new Node(t, LIST, n[1]);
                else // uh, was he a GROUP? did the comma do something? whatevs.
                    n[1].type = LIST;
                // okay, now let's make sure we create nodeLinks for all of those
                //  argument bits.  idempotent clobbering may happen; s'ok
                var listNode = n[1];
                for (var iNode = 0; iNode < listNode.length; iNode++) {
                  if (listNode[iNode].synToken)
                    listNode[iNode].synToken.nodeLink = listNode[iNode];
                }
            }
            --x.parenLevel;
            break;
...
var RE_JUST_WHITESPACE = /^[ ]+$/;
exports.parseUntilRightCurly = function(src, filename, lineNo, elideList,
                                        normalizeWhitespace, noKillLast) {
  var t = new Tokenizer(src, filename, lineNo, true, elideList);
  var x = new CompilerContext(false);
 // (catch exceptions to decorate the exception on how far we got)
 try {
  // actually parse in here (stopping at the first excess right curly)
  var n = Script(t, x);

  // kill the right curly...
  if (!noKillLast)
    t.tokenLog.splice(t.tokenLog.length - 1, 1);

  // end before the terminating right squiggly brace
  var idxStart = 0, idxEnd = t.cursor - 1;

  if (normalizeWhitespace) {
    var tokens = t.tokenLog;
    var whitespaces = t.whitespaceFixupIndices;

    // step 1: if the very first thing is a newline, kill it by replacing it
    //  with an empty string (to avoid shifting later passes.
    if (tokens[0] === "\n") {
      tokens[0] = "";
      if (whitespaces.length && whitespaces[0] == 0)
        whitespaces.splice(0, 1);
    }
    // step 2: if the very last thing is whitespace, kill it and take off
    //  the newline on the preceding line too.
    var lastIndex = tokens.length - 1, lastWhitespace = whitespaces.length - 1;
    if (RE_JUST_WHITESPACE.test(tokens[lastIndex])) {
      // (remove the whitespace entry too, if there is one)
      if (lastWhitespace >= 0 &&
          (Math.abs(whitespaces[lastWhitespace]) == lastIndex))
        whitespaces.splice(lastWhitespace, 1);
      // (kill and eat newline)
      tokens.splice(lastIndex--, 1);
      tokens[lastIndex] = tokens[lastIndex].slice(0, -1);
    }
    // alternatively, it could have been a negative marker that references the
    //  closing squiggly we sliced out, which indicates it was whitespace.
    //  same deal to handle it.
    else if (lastWhitespace >= 0 &&
             (whitespaces[lastWhitespace] == -tokens.length)) {
      whitespaces.splice(lastWhitespace, 1);
      tokens[lastIndex] = tokens[lastIndex].slice(0, -1);
    }

    // step 3: pass 1 over whitespace indices, find the shortest whitespace
    //  used.
    // (a negative index means the thing at the (positive) index is the first
    //  thing on a new line and had no whitespace.  we bother tracking this at
    //  all so step 2 above can confidently nuke the whitespace entry affiliated
    //  with the last line...)
    var i, wsi, wsl;
    if (whitespaces.length) {
      var shortest = (whitespaces[0] === -1) ? 0
                                             : tokens[whitespaces[0]].length;
      for (i = 1; i < whitespaces.length; i++) {
        wsi = whitespaces[i];
        if (wsi < 0) {
          shortest = 0;
          break;
        }
        wsl = tokens[wsi].length;
        if (wsl < shortest)
          shortest = wsl;
      }
      // step 4: kill that much whitespace off all other fixups.
      if (shortest) {
        for (i = 0; i < whitespaces.length; i++) {
          // no one is going to be -1 or shortest would have been zero.
          wsi = whitespaces[i];
          tokens[wsi] = tokens[wsi].substring(shortest);
        }
      }
    }
  }
...
/**
 * Check the given string for correctness as a valid JS bunch'o'statements.
 *
 * Returns null on success, a description of the problem on failure.
 */
exports.syntaxCheck = function(src) {
  var t = new Tokenizer(src, "*syntaxcheck*", 1, /* retain tokens */ false);
  var x = new CompilerContext(false);
  var n;
