...
exports.Interpreter.prototype = {
    ...
    _exec: function(node, ctx) {
        ...
        case tokenIds.ARRAY_INIT:
          // XXX the only kind of array we would carry about is one where it is
          //  being used to return multiple return values and is immediately
          //  unboxed on the other side in some way we care about.
          // We punt for now until that turns out to happen to something we care
          //  aboot.
          return this.getNullValue();
...
function docRequire(interp, ctx, thisObject, args) {
  ...
  var minfoOrPromise = sandbox.docFusion.requireModule(moduleName, baseId);
  // Promises, like vegetables, are death.  The InterpSandbox will catch this,
  //  wait on the promise, then restart interpretation when the promise is
  //  fulfilled.  Disclaimers live up there.
  if (pwomise.isPromise(minfoOrPromise))
    throw minfoOrPromise;
  return minfoOrPromise.rawExportsScope;
}
