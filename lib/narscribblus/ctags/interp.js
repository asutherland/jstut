...
exports.InterpSandbox.prototype = {
   ...
   */
  processAnonSnippet: function(preAsts, realAst, name) {
    dump("*** INTERP " + name + "\n");
    var interp = new exports.Interpreter({commonJS: true, sandbox: this});
...
exports.Interpreter.prototype = {
        ...
        case tokenIds.OBJECT_INIT:
            ...
            node.forEach(function(init) {
                ...
                case tokenIds.PROPERTY_INIT:
                    ...
                    if (this.annotateTerms) {
                      init.interpObj = ["attr", {owner: node,
                                                 name: name}];
                    }
                    break;
    ...
    // XXX transient refactoring state, do not keep ftloatih
    allDoneDoStuff: function() {
        var opts = this.opts;
        if (!opts.commonJS) {
            var scope = ctx.scope;
            while (scope !== null) {
          ...
          console.log("toplevel", this.ctx.scope.object);
