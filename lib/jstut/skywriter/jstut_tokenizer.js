...
/**
 * JS tokenizer backed by our JS parse/abstract interpretation logic and which
 *  can use data extracted from the full power of the jstut infrastructure AKA
 *  both other .js source files and .jstut files.
 *
 * XXX this may be slightly bit-rotted, or at least if will be if we update our
 *  ace submodule dep.
 **/
 ...
 *
 */
function JstutTokenizer(fallbackTokenizer, interpbox) {
  this.tokenLines = [];
...
JstutTokenizer.prototype = {
  ...
  parse: function(lines) {
    ...
    try {
      ...
      this.interpbox.processAnonSnippet(this.preAsts,
                                        parsed.script,
                                        "#skywriter#");
  ...
  getAutocompleteInfoAt: function(iRow, iCol) {
    ...
    if (this.lastGoodTokenLines === this.tokenLines) {
      ...
      if (leftChar !== " ") {
        ...
        typedSoFar = token.value.substring(relOff.start,
                                           relOff.value + 1);
        return {
