...
JstutTokenizer.prototype = {
  ...
  parse: function(lines) {
    ...
    try {
      ...
      for (var i = 0; i < tokenLog.length; i++) {
        ...
        // XXX we may need to fragment multi-line strings too; STRING type
      ...
      this.env.interpbox.processAnonSnippet([], parsed.script, "#skywriter#");
  ...
  getTokenInfoAt: function(iRow, iCol) {
    if (iRow >= this.tokenLines.length)
      return null;
    var line = this.tokenLines[iRow];
    var iToken = 0, token = null;
    while (iCol >= 0 && iToken < line.length) {
      ...
      if (typeof(token) === "string") {
        ...
        iCol -= ("" + token.value).length;
      }
    }
    if (token && ("trueToken" in token))
