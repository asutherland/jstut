...
/**
 * Dump info on the token at the currsor location.
 */
$canon.addCommand({
  name: "jstut-info",
  exec: function(env, args, request) {
    var pos = env.editor.getCursorPosition(); // {row, column}
    var parser = env.editor.bgTokenizer.parser;
...
$canon.addCommand({
  ...
  exec: function(env, args, request) {
    ...
    // - show a popup!
    var cursorNode = env.editor.renderer.$cursorLayer.cursor;
    console.log("cursorNode", cursorNode);
    env.editor.wmsyBinding.showAutocomplete(cinfo, cursorNode);
  },
});
...
exports.injectIntoEditor = function injectIntoEditor(editor) {
  editor.keyBinding.keyNames["191"] = "/";
  var config = editor.keyBinding.config;
  delete config.reverse;
  config["jstut-info"] = "Ctrl-/";
  config["jstut-complete"] = "Alt-/";
  editor.keyBinding.setConfig(config);
};
