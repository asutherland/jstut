...
const AtCommand = $syn.AtCommand;


/**
 * Traverse the `AtCommand` instances in a list of nodes, calling
 *  processing functions when there is one registered for the command name,
 *  and optionally recursing.  No mutation or results are generated.
 *
 * This function is intended to be used to allow a dependency gathering pass
 *  to be introduced where needed for cases where an acyclic dependency
 *  graph is possible and commands can benefit from having immediate access
 *  to fully processed dependencies (rather than having to create references
 *  that are only resolved during the processing phase/on demand.)
 */
function visitAtStream(nodes, processingFuncs, ctx, recurse) {
  for (var i = 0; i < nodes.length; i++) {
    var node = nodes[i];
    if (node instanceof AtCommand) {
      if (node.name in processingFuncs) {
        processingFuncs[node.name](node.name, node.svals, node.textStream, ctx);
      }
      if (recurse) {
        if (node.svals)
          visitAtStream(node.svals, processingFuncs, ctx, recurse);
        if (node.textStream)
          visitAtStream(node.textStream, processingFuncs, ctx, recurse);
      }
    }
  }
}
exports.visitAtStream = visitAtStream;
    ...
    if ((typeof(node) !== "object") || (node == null)) {
      ...
      if (node.name in ctx.funcMap) {
        ...
        if (node.name in ctx.funcPreMap) {
          preVal = ctx.funcPreMap[node.name](node.name, ctx, node.index);
        }
