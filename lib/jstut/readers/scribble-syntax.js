 ...
 *
 * In a nutshell, it looks like: ATSIGNcmd[sexpr1 sexpr2]{text text text}
 *
 ...
 *  (We have no quasiquote, no reader macro infrastructure, etc.)  What I mean
 *  is that you could have any of "(require blah)" or "(lambda (x) x)" as
 *  commands, not to mention quoting syntax, etc. and we cannot understand
 *  any of that, nor do we want to.
 **/
...
exports.oneOfKeywords = function oneOfKeywords(thing) {
...
function chewTextFileForLinesAndStuff(bodyString, idx) {
  ...
  var lines = bodyString ? bodyString.split("\n") : [""];
...
function AtCommand(name, index, svals, textStream) {
  this.name = name;
  /**
   * The character-offset of the start of the command within its file.
   */
  this.index = index;
  this.svals = svals;
...
function atParser(s, ctx, idx, re_at_or_term) {
  ...
  ctx.bumpUnconsumedIndex(idx - relIdx);
  return [new AtCommand(cmdName, cmdIndex, cmdSVals, cmdTextStream), idx];
}
