...
function decodeFlow(strOrNodes) {
  ...
  function maybePush(s) {
    if (typeof(s) === "string") {
      // leave the whitespace intact for this check...
      var trimmed = s.trim();
      if (trimmed.length === 0)
        return;
    }
...
exports.process = function process(nodes, ctx) {
  // er, we really want people to be using forcelang=jstut/raw?
  /*
  if (ctx.options.mode == "raw") {
    return {
...
/**
 * An inline link to the internet-thing that should be treated as part of a text
 *  run like a normal hyperlink.  Compare with a `TopicLink` which is intended
 *  to be extracted/elided from the document stream or otherwise set apart.
 *
 * We just generate a straight-up "a" "href" tag to minimize user frustration.
 */
function WebLink(svals, tvals, ctx) {
  this.href = svals[0];
  this.kids = tvals;
}
WebLink.prototype = {
...
exports.jstutExecFuncs = {
  ...
  // -- linking
  weblink: function(name, svals, tvals, ctx) {
    return new WebLink(svals, tvals, ctx);
  },
