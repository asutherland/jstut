...
/**
 * XXX This was never completed and the jetpack API syntax gained some
 *  improvements after this work was done.  Specifically, I believe the
 *  context is now explicit.  It shouldn't be too hard to complete that work,
 *  but the presentation bit likely requires more love.  I am currently
 *  ripping out the deprecated HTML logic too.
 *
...
function blockSplitter(s, ctx) {
  var minfo = ctx.metaInfo, raw = ctx.rawMode;
  if (raw)
    ctx.tokenRuns = [];
...
function processRaw(parsed, ctx) {
  ...
  // they should use token runs, really
  throw new Error("XXX this does't work, and the bit above is ill-advised");
}
...
function processDocStream(parsed, ctx) {
  throw new Error("XXX not a thing");
}
