 ...
 *    interesting.
 *
 * (Theoretically) Supported modes:
 * - raw: wmsy, just JS syntax highlighted by wrapping the code in a syntax
 *    block, no comment parsing, avoids explicit docfusion fetch.
 * - rawcomments: html, JS syntax highlighted, attempted comment parsing with
 *    its own highlighting stuff.  will docfusion fetch.
 * - source: wmsy, interleaved source with doc blocks
 * - illuminated: html, literate programming style, currently broken.
 * - modinfo: wmsy, type representations of the namespace
 * - meta: raw metadata for docfusion purposes, not for display!
 **/
...
var ModuleInfo = docfusion.ModuleInfo;
...
exports.parse = function parse(s, ctx) {
  ...
  // In rawcomments mode we want all tokenizers to generate token runs.
  if (ctx.options.mode === "rawcomments") {
    ctx.nestedRawMode = true;
    ctx.rawMode = true;
    ctx.tokenRuns = [];
  }
...
exports.expand = function expand(parsed, ctx) {
  var options = ctx.options, minfo = ctx.metaInfo;
  ...
  if (!parsed) {
    ...
    minfo.rawExportsScope = {type: "object", data: {}};
    return minfo;
  }
  ...
  var interpbox = new astinterp.InterpSandbox(ctx.docFusion);
  return when(interpbox.processModule(parsed.script,
                                      minfo.name),
              function interpProcessed(interp) {
...
/**
 * Wraps a text stream in an object that can be linked to a `Symish` node and
 *  also be associated with a `Grouping`.
 */
 ...
 *  `Symish` mungers see a group-associated `Symish` child, they create an
 *  `InstantiatedGrouping` to actually hold the named children.  They do not use
 *  us directly because we are not clever enough, given our syntax for grouping
 *  blocks, to make sure Grouping objects are constrained to a single parent
 *  container.
 */
 ...
 */
function parseDocBlockAsScribble(s, ctx, relLine) {
  try {
    if (ctx.nestedRawMode)
      ctx.tokenRuns = [];
    var parsed = $man.parse(s, ctx);
    // although parsing is nominally synchronous, it obeys our promise idiom
    //  and so will convey failure with a rejected promise
    if ($pwomise.isRejected(parsed)) {
      ctx.logParseWarning(parsed, relLine);
      return null;
    }
    var rawStream = $docstreams.textStreamChewer(parsed, ctx);
    var formattedStream = ctx.formatTextStream(rawStream);
    var docNode = new DocNode(rawStream, formattedStream);
    if (ctx.nestedRawMode) {
      docNode.tokenRuns = ctx.tokenRuns;
      ctx.tokenRuns = null;
    }
    return docNode;
  }
  catch (ex) {
    ctx.logParseWarning(ex, relLine);
  }
  return null;
}

/**
 * Process the comment block as a jstut scribble language bit.
 */
function chewCommentBlock(blockStr, ctx, relLine) {
  blockStr = blockStr.replace(RE_DOCBLOCK_TRIMMER, "");
  return parseDocBlockAsScribble(blockStr, ctx, relLine);
}
...
function chewGroupLines(contentTokens, ctx) {
  ...
  var docNode = parseDocBlockAsScribble(blockStr, ctx, contentTokens[0].lineno);
 ...
 *  detect the `CBLOCK_GROUP_START`/`CBLOCK_GROUP_END` grouping syntax
    ...
    if (token.type === COMMENT_BLOCK) {
      ...
      if (blockType < FIRST_DONT_CARE_CBLOCK) {
        var docNode = token.rawDocNode = chewCommentBlock(token.value, ctx,
                                                          token.lineno);
        if (docNode && activeGroup)
          docNode.group = activeGroup;
        // ignore doc nodes we could not parse; steal control flow.
        if (!docNode) {
        }
        else if (blockType === CBLOCK_PRE) {
...
/**
 * Traverse the token stream, wrapping runs of JS tokens in JS blocks (to get
 *  us syntax highlighting) and explicitly emitting the documentation nodes
 *  associated with comment blocks that separate those blocks.
 */
    ...
    if (token.type === COMMENT_BLOCK ||
      ...
      if (!token.inlineComment) {
        ...
        if ("rawDocNode" in token) {
          var rawDocNode = token.rawDocNode;
          if (ctx.nestedRawMode) {
            if (rawDocNode.tokenRuns.length > 1)
              nodes = nodes.concat(rawDocNode.tokenRuns);
            else if (rawDocNode.tokenRuns.length)
              nodes.push(rawDocNode.tokenRuns[0]);
          }
          else {
            nodes.push(rawDocNode);
          }
        }
...
/**
 * We have modified the jsCommentSlicer to notice when we are in nestedRawMode
 *  and to accordingly splice the token runs in instead of the raw doc node.
 */
function processRawWithComments(parsed, ctx) {
  return {
    app: "doc",
...
/**
 * Parse the comment blocks and replace them with their jstut scribble
 *  parsed HTML output.  We use `jsCommentSlicer` to drive this.
 */
...
function processIlluminated(parsed, ctx) {
  throw new Error("XXX this needs to enter the wmsy age");
  // noting that it should not be particularly hard to bring this up-to-date
  ...
  // we want to return an app here, etc.
}
...
exports.process = function process(parsed, ctx) {
  switch (ctx.options.mode) {
    ...
    case "rawcomments":
      return processRawWithComments(parsed, ctx);
    case "source":
