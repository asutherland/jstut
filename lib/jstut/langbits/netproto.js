...
/**
 * Container for network protocol documentation, both strict structured
 *  data types and a free-form explanation that references those types.
 */
 ...
 *
 */
function ProtoParticipants(netproto, participants) {
  this.netproto = netproto;
  /**
   * @listof[@oneof[Actor ActorGroup]]
   */
  this.participants = participants;
}
ProtoParticipants.prototype = {
  kind: 'proto-participants',
};
...
function MessageDef() {
  ...
  /**
   * @listof[MessageDef]{
   *   Messages that extend our message type.
   * }
   */
  this.childTypes = [];
...
exports.jstutExecFuncs = {
   ...
   *
   * (In reality this is just an organizational node that exists to provide a
   *  single documenter-defined location in the protocol definition to list the
   *  participants.)
   *
   * @args[
   *   @param[svals @list[
  ...
  protoParticipants: function(tagName, svals, tvals, ctx) {
    return new ProtoParticipants(ctx.lookupNamedValue('netproto'), svals);
  },
  /**
  ...
  actor: function(tagName, svals, tvals, ctx) {
    var name = coerceString(svals[0]),
        ...
        idx = 2;
    while (oneOfKeywords(svals[idx], 'icon')) {
      var keyword = svals[idx++].keyword;
      switch (keyword) {
        case 'icon':
          actor.icon = coerceString(svals[idx++]);
          break;
      }
    }
    actor.docStream = ctx.formatTextStream(tvals);
  ...
  pluralActor: function(tagName, svals, tvals, ctx) {
    var singularName = coerceString(svals[0]),
        ...
        idx = 2;
    while (oneOfKeywords(svals[idx], 'singularIcon', 'pluralIcon')) {
      var keyword = svals[idx++].keyword;
      switch (keyword) {
        case 'singularIcon':
          actor.icon = coerceString(svals[idx++]);
          break;
        case 'pluralIcon':
          group.icon = coerceString(svals[idx++]);
          break;
      }
    }
  ...
  actorGroup: function(tagName, svals, tvals, ctx) {
    var pluralName = coerceString(svals[0]),
        ...
        members = [], role, idx, member, memberName, sval,
        singularIcon, pluralIcon;
    for (idx = 1; idx < svals.length; idx++) {
      sval = svals[idx];
      if (sval instanceof Keyword) {
        switch (sval.keyword) {
          case 'singularIcon':
            singularIcon = coerceString(svals[++idx]);
            break;
          case 'pluralIcon':
            pluralIcon = coerceString(svals[++idx]);
            break;
          default:
            throw new Error("Unsupported keyword: " + sval.keyword);
        }
        continue;
      }
      memberName = coerceString(sval);
    ...
    if (singularIcon)
      actor.icon = singularIcon;
    if (pluralIcon)
      group.icon = pluralIcon;
  ...
  boxedMessage: function(tagName, svals, tvals, ctx, msg) {
    ...
    msg.docStream = ctx.formatTextStream(tvals);
