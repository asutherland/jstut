...
exports.jstutPreExecFuncs = {
  dict: function(tagName, ctx) {
    ...
    ctx.pushNamedContext("dict-all", dict.childrenByName);
    ctx.pushNamedContext("groups-container", dict.groups);
    ctx.pushNamedContext("dict-group", dict.ungroupedChildrenByName);
...
exports.jstutExecFuncs = {
  ...
  typedef: function(tagName, svals, tvals, ctx) {
    ...
    var type = chewType(name, svals[1], ctx),
        typedef = new $typerep.Typedef(name, type, ctx.formatTextStream(tvals));
    defineType(ctx, typedef);
    return typedef;
  },
  /**
   * @args[
  ...
  key: function(tagName, svals, tvals, ctx) {
    ...
    // should be dict/group (inside a dict) or something acting like one
    else {
  ...
  protocol: function(tagName, svals, tvals, ctx) {
    ...
    var type = (svals.length >= 2) ? chewType(null, svals[1], ctx) : null,
        proto = new $protocols.Protocol(name, type,
                                        ctx.formatTextStream(tvals));
    defineType(ctx, proto);
    return proto;
  },
