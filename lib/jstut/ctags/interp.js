...
/**
 * The purpose of this interpreter in the context of jstut is to be able
 ...
 *  the explicitly written documentation in all cases.
 *
 * Because we want to avoid gratuitous manual annotations, we do require our
 *  interpeter to:
 * @itemize[
 *   @item{
 *     Load and interpret to the same approximation all required modules.
 *   }
 *   @item{
 *     Track propagation of values with their associated documentation blocks.
 *   }
 * ]
 *
 *
 * @typedef[InterpNode @dict[
 *   @key[node Node]{
 *     The narcissus Node
 *   }
 *   @key[type @oneof[
 *     @case["list"]{
 *       A list declaration, potentially used for function call arguments.  Its
 *       arguments are evaluated and placed in `data`.
 *     }
 *     @case["ref"]{
 *       Attribute lookup on key `container` using key `name`; could be explicit
 *       (foo.bar) or implicit (lexical lookup).  `name` is always a String.
 *     }
 *     @case["ref:index"]{
 *       Attribute lookup on key `container` using key `index` where `index`
 *       is itself an InterpNode.
 *     }
 *     @case["undefined"]{
 *       An undefined value.  Accompanied by a null `data`.
 *     }
 *   ]]{
 *   }
 *   @key[data Object]{
 *     The value associated with the result of interpreting a node.  This could
 *     be a primitive (number/string/boolean), complex built-in (RegExp), array
 *     or rich object.
 *   }
 *   @key[container #:optional InterpNode]{
 *     Used in 'ref' nodes to refer to the InterpNode we are pulling the value
 *     out of.  This could be as part of an explicit (foo.bar) lookup or
 *     implicit (lexical lookup).
 *   }
 *   @key[name #:optional String]{
 *     Used by ref nodes to describe the key value used to perform attribute
 *     lookup on the `container`.
 *   }
 *   @key[index #:optional InterpNode]{
 *     The "bar" in foo[bar] style attribute indexing in 'ref:index' nodes.
 *   }
 * ]]{
 *   The representation type used by the abstract interpretation engine.
 * }
 *
 * @typedef[InterpScope @dict[
 *   @key[object InterpNode]
 *   @key[parent InterpScope]
 * ]]
 *
 * @typedef[InterpObj @oneof[
 *   @case[@list["arglist" @dict[@key[func InterpNode]]]]{
 *     Links to the function which we are the argument list of.
 *   }
 *   @case[@list["arg" @dict[@key[owner InterpNode] @key[index Number]]]{
 *     Annotated onto the arguments of a function call, this references the
 *     list `InterpNode` which should have an "arglist" InterpObj on its node.
 *     The index is our index in the argument list.
 *   }
 *   @case[@list["attr" @dict[
 *     @key[owner Node]{
 *       The OBJECT_INIT node to which this attribute definition belongs.
 *     }
 *     @key[name String]]{
 *       Our attribute name.
 *     }
 *   ]]
 *   @case[@list["attrval" @dict[
 *     @key[attr Node]]{
 *       A link to the naming node which has an "attr" interpObj on it.
 *     }
 *   ]]
 *   @case[@list["new" InterpNode]]{
 *     The thing that is getting newed.
 *   }
 *   @case[@list["ref:dot" InterpNode]]{
 *     A dotted lookup against something.
 *   }
 *   @case[@list[
 *     "ref:scope"
 *     @param["reference" InterpNode]{
 *       The reference node whose container is the container of where our
 *       lookup found the name.
 *     }
 *     @param["scope obj" InterpScope]{
 *       The most-specific scope this reference lives in; this was the starting
 *       scope in our search.
 *     }
 *   ]]{
 *     A lexical scope lookup; the
 *   }
 *   @case[@list["ref:index" InterpNode]]{
 *     foo[bar] style attribute lookup.
 *   }
 *   @case[@list[
 *     "scope"
 *     @param["scope obj" InterpScope]{
 *       The scope introduced on this SCRIPT (or block when LET shows up.)
 *     }
 *   ]]{
 *     Annotated onto SCRIPT nodes to link them to their scope objects.
 *   }
 * ]]{
 *   A metadata object that is annotated onto token `Node` instances for the
 *   purpose of being able to generate a traversal path that allows us to
 *   linkify the syntax-highlighted source to the relevant code documentation.
 * }
 **/

define("jstut/ctags/interp",
  [
    "exports",
    "jstut/utils/pwomise",
...
exports.InterpSandbox = function InterpSandbox(docFusion) {
  if (!docFusion)
    throw new Error("You need to provide a docFusion instance!");
...
exports.Interpreter.prototype = {
    ...
    _deref: function(value) {
        var name, container;
        // indexes may defy abstract interpretation; we can only directly
        //  resolve string and numeric types.
        if (value.type === 'ref:index') {
            if (value.index.type === 'number' ||
                value.index.type === 'string') {
                name = value.index.data + "";
    ...
    _exec: function(node, ctx) {
        ...
        case tokenIds.SCRIPT:
            ...
            if (this.annotateTokens)
              node.interpObj = ["scope", ctx.scope];
        ...
        case tokenIds.DOT:
            ...
            if (this.annotateTokens)
                node.interpObj = ["ref:dot", rv];
            return rv;

        // XXX I just added the INDEX case and it's possible it does not work.
        // (The goal is not to make it work so much as make sure we get our
        //  interpObj on there.)
        case tokenIds.INDEX:
            lhs = exec(node[0]);
            container = deref(lhs);
            rhs = deref(exec(node[1]));
            rv = {
                type: 'ref:index',
                container: container,
                index: rhs,
        ...
        case tokenIds.IDENTIFIER:
            ...
            if (this.annotateTokens)
                node.interpObj = ["ref:scope", rv, ctx.scope];
            return rv;
        ...
        case tokenIds.RETURN:
            ...
            //  jstut came to town; filling it out / running with it
            //  for now.
    ...
    setupContext: function() {
        ...
        if (!opts.commonJS) {
            ...
            var require = {
              ...
              nativeFn: docRequire,
              data: {
                // require.def is now just an alias for define()
                def: {
                  type: 'function',
                  nativeFn: docDefine,
                }
...
function docRequire(interp, ctx, thisObject, args) {
  ...
  if (depPromises.length) {
    if (depPromises.length === 1)
      throw depPromises[0];
    throw pwomise.all(depPromises, null, "require", modulesNames.join(", "));
  }
...
/**
 * An implementation of define as used by RequireJS or other compatible
 ...
 * ]
 */
function docDefine(interp, ctx, thisObject, args) {
  var moduleNameInterp, depsInterp, funcInterp;

  // (id, deps, factory) or (id, factory)
  if (args.data[0].type === "string") {
    moduleNameInterp = args.data[0];
    // (id, deps, factory)
    if (args.data[1].type === "list") {
      depsInterp = args.data[1];
      funcInterp = args.data[2];
    }
    // (id, factory)
    else {
      depsInterp = null; // will trigger regex below
      funcInterp = args.data[1];
    }
  }
  // (deps, factory) or (factory)
  else {
    moduleNameInterp = null; // we don't actually care.
    // (deps, factory)
    if (args.data[0].type === "list") {
      depsInterp = args.data[0];
      funcInterp = args.data[1];
    }
    // (factory)
    else {
      depsInterp = null; // will trigger regex below
      funcInterp = args.data[0];
    }
  }
  // XXX handle the object literal case...

  // do the regexp on the function body if relevant.
  if (depsInterp == null) {
    // XXX actually do this in the future...
    // (the require logic in docRequire will handle this, but it's semantically
    //  different...)
    depsInterp = {data: []};
  }
  ...
  if (true) {
    for (var i = 0; i < depsInterp.data.length; i++) {
      ...
      // RequireJS plugin?
      else if (depModuleName.indexOf("!") != -1) {
        var idxFirstBang = depModuleName.indexOf("!");
        var pluginName = depModuleName.substring(0, idxFirstBang);
        // for text plugins, return an empty string
        if (pluginName === "text")
          moduleExports.push({type: 'string', data: ""});
        // For everything else, give it an empty object because we don't know
        //  what they allege to do.
        else
          moduleExports.push({type: 'object', data: {}});
        continue;
      }
    ...
    if (depPromises.length)
      throw pwomise.all(depPromises, null, "define");
