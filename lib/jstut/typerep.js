...
/**
 * Unified type representation scheme for the two big producers of types:
 * @itemize[
 *   @item{
...
exports.commonTraverseChild = commonTraverseChild;
 ...
 *   @item{All owners, where owner is defined as an object we were set as
 *         a property on.  For example, if we are just a helper function,
 *         we might end up exposed on our module's "exports" value, as well
 *         as another module that serves as the public API exposure and so
 *         re-exports us.}
 * ]
 *
...
function LifeStory(modInfo, line) {
  ...
  /**
   * If positive, the 1-based line number we are located on.  If zero, it
   *  means we don't know.  If negative, it means we are a character-based
   *  index in the file (whose parser only deals in indices and for which
   *  computing lines would be expensive without a code change.)
   */
 ...
 *
 * XXX Needs specialized presentation widget.
 */
function PropertyDescriptor(name, type, getter, setter) {
  this.name = name;
  /**
   * The caller is reponsible for trying to figure out our type by looking
   *  at the getter and/or setter and/or explicit annotations.  Ideally,
   *  no one will look at this type and will instead directly expand out to
   *  getters and setters.
   */
  this.type = type;
  this.getter = getter;
